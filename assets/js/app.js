...
(function(){
  ...
  const ENDPOINT = $('#endpointView')?.textContent.trim() || 'https://sheetdb.io/api/v1/avsi1ki6gcrlr';
  ...
  async function computeMonthly(){
    ...
    try {
      ...
      const rows = data.filter(r => (r.tipo === 'pedido' || r.tipo === 'factura') && inCurrentMonth(r.fecha));

      let capital = 0, ganancia = 0;
      for (const r of rows) {
        const valor = parseFloat(r.valor ?? r.total ?? 0);
        const libra = parseFloat(r.libra ?? 0);
        if (isFinite(valor)) capital += valor;
        if (isFinite(valor) && isFinite(libra)) ganancia += valor * (libra/100);
      }
      animateValue(elCap, capital, 1000);
      animateValue(elGan, ganancia, 1100);
    } catch (err) {
      console.error('Resumen mensual v3.7.1 error:', err);
    }
  }
  computeMonthly();
})();
